...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	void swap(int a,int b){
		Decreaser<T> temp = array[a];
		int aloc = array[a].loc;
		int bloc = array[b].loc;
		array[a] = array[b];
		array[b] = temp;
		array[a].loc = aloc;
		array[b].loc = bloc;
	...
	void decrease(int loc) {
		...
		if(size>1){
			...
			while(child > 1 && (array[child].getValue().compareTo(array[child/2].getValue())<0))
			{
				swap(child,child/2);
				child = child/2;
				ticker.tick(2);
	...
	private void heapify(int where) {
		...
		int lchild = where*2;
		int rchild = where*2+1;
		ticker.tick(2);
		if(lchild<=size&&rchild>size){//if left child exists
			if (array[lchild].getValue().compareTo(array[where].getValue())<0){
				swap(lchild,where);
				heapify(lchild);
				ticker.tick(2);
			}
		}
		else if(rchild<=size){//if right child exists
				if(array[lchild].getValue().compareTo(array[where].getValue())<0 ||array[rchild].getValue().compareTo(array[where].getValue())<0){
					if (array[lchild].getValue().compareTo(array[rchild].getValue())<0){
						swap(lchild,where);
						heapify(lchild);
						ticker.tick(2);
					}
